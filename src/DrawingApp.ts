...
export class DrawingApp extends gfx.GfxApp
{
    ...
    update(deltaTime: number): void
    {
        ...
        if (this.drawState == DrawState.NO_DRAWING) {
            ...
            // For the final part of this assignment, you will add some code to the update() method of the DrawingApp class. Currently,
            // the user can move the camera using the WASD keys, and the height always remains at a fixed value. 
            // This means that if the user draws a hill, then they could end up walking into and even underneath the ground mesh.
            // Instead, we want to adjust the camera height dynamically based on the elevation of the ground at the location of the camera.
            // To find the right Y height for the camera, you will need to perform a ray cast directly downwards (-Y) from the camera's position in X and Z,
            // figure out where that ray intersects the ground mesh, and then set the camera's hight based on this.
            ...
            if (this.cameraControls.hasMoved) {
                ...
                const direction = gfx.Vector3.DOWN; // downward dicrection
                const userPosition = this.camera.position; // user's position
                // Cast a ray downwards from the user's position to find the intersection point with the ground mesh.
                const downwardRay = new gfx.Ray3(); // Casting a ray downwards 
                downwardRay.set(userPosition, direction) // set the origin of the ray and the direction

                // find the intersection point with the ground mesh.
                const interesectinPoint = downwardRay.intersectsTriangles(this.ground.vertices, this.ground.indices);
                if(interesectinPoint){
                    // Adjust the height of the camera to be the correct distance above the ground mesh.
                    const aboveGround = interesectinPoint.y + this.cameraHeight; // distance above the ground
                    this.camera.position.y = aboveGround; // adjust the height of the camera to be above the ground
                }
            }
        }
    }
